...
export default function GeneralFeed() {
  ...
  const categories = Array.from(
    new Set(mockGeneralPosts.flatMap(post => post.categories || []))
  );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Allgemein</Text>
        <TouchableOpacity 
          style={styles.addButton}
          onPress={() => navigation.navigate('CreateGeneral')}
        >
      ...
      {sortedPosts.length === 0 ? (
        ...
        <FlatList
          data={sortedPosts}
          renderItem={({ item }) => <GeneralPostCard post={item} />}

...
export default function EditNewsScreen({ route, navigation }) {
  ...
  const { control, handleSubmit, watch, formState: { errors }, reset } = useForm({
    ...
    defaultValues: {
      ...
      hasVoting: post.hasVoting || false,
  ...
  useEffect(() => {
    ...
    setVotingOptions(post.voting?.options.map(o => o.text) || ['Ja', 'Nein']);
  }, [post, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.log('Updating post with:', { ...data, mediaPreview, votingOptions });
      ...
      Alert.alert(
        ...
        [{ text: 'OK', onPress: () => navigation.navigate('NewsDetail', { postId }) }]
      );
    } catch (error) {
